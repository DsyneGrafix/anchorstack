...
import { Coins, TrendingUp, Crown, Gift, Star } from 'lucide-react';
...
export const QuickTokenActions = () => {
    ...
    return (_jsx(Card, { children: _jsxs(CardContent, { className: "p-4", children: [_jsxs("div", { className: "flex items-center justify-between mb-3", children: [_jsx("h3", { className: "font-semibold text-anchor-800", children: "Quick Actions" }), _jsxs(Link, { to: "/tokens", className: "text-primary-600 hover:text-primary-700 text-sm flex items-center space-x-1", children: [_jsx("span", { children: "View All" }), _jsx(ArrowRight, { className: "w-3 h-3" })] })] }), _jsx("div", { className: "space-y-2", children: quickActions.map((action) => {
                        const IconComponent = action.icon;
                        return (_jsxs("button", { onClick: action.action, disabled: !action.canEarn, className: `
                  w-full flex items-center justify-between p-3 rounded-lg border transition-colors text-left
                  ${action.canEarn
                                ? 'border-green-200 bg-green-50 hover:bg-green-100'
                                : 'border-gray-200 bg-gray-50 cursor-not-allowed'}
                `, children: [_jsxs("div", { className: "flex items-center space-x-2", children: [_jsx(IconComponent, { className: `w-4 h-4 ${action.canEarn ? 'text-green-600' : 'text-gray-400'}` }), _jsx("span", { className: `text-sm font-medium ${action.canEarn ? 'text-green-800' : 'text-gray-600'}`, children: action.title })] }), _jsxs("span", { className: `text-xs font-bold ${action.canEarn ? 'text-green-600' : 'text-gray-400'}`, children: ["+", action.tokens] })] }, action.id));
                    }) })] }) }));
};
// src/components/TokenRedemptions.tsx - Vault Integration
import { useState } from 'react';
import { ShoppingCart, Download, Trophy, Zap } from 'lucide-react';
export const TokenRedemptions = () => {
    ...
    const handleRedeem = async (redemption) => {
        if (user.tokens < redemption.cost)
            return;
        if (redemption.minTier && getTierLevel(user.tier) < getTierLevel(redemption.minTier))
            return;
    ...
    return (_jsxs(Card, { children: [_jsx(CardHeader, { children: _jsx(CardTitle, { children: "Redeem Tokens" }) }), _jsx(CardContent, { children: _jsx("div", { className: "space-y-4", children: redemptions.map((redemption) => {
              ...
              `, children: [_jsxs("div", { className: "flex items-center space-x-3", children: [_jsx(IconComponent, { className: `w-5 h-5 ${canRedeemThis ? 'text-blue-600' : 'text-gray-400'}` }), _jsxs("div", { children: [_jsxs("div", { className: "flex items-center space-x-2", children: [_jsx("h4", { className: `font-medium ${canRedeemThis ? 'text-blue-800' : 'text-gray-600'}`, children: redemption.title }), redemption.minTier && (_jsxs("span", { className: "text-xs bg-purple-100 text-purple-600 px-2 py-0.5 rounded", children: [redemption.minTier, "+ only"] }))] }), _jsx("p", { className: `text-sm ${canRedeemThis ? 'text-blue-600' : 'text-gray-500'}`, children: redemption.description })] })] }), _jsxs("div", { className: "flex items-center space-x-3", children: [_jsxs("span", { className: `font-bold ${canRedeemThis ? 'text-blue-600' : 'text-gray-400'}`, children: [redemption.cost, " tokens"] }), _jsx(Button, { onClick: () => handleRedeem(redemption), disabled: !canRedeemThis || isRedeeming, size: "sm", variant: canRedeemThis ? 'default' : 'outline', children: isRedeeming ? 'Processing...' : canRedeemThis ? 'Redeem' : 'Cannot Redeem' })] })] }, redemption.id));
                    }) }) })] }));
};
...
export default async function handler(req, res) {
    ...
    try {
        const { email, price, product_id, product_name, sale_id, test } = req.body;
...
export const HowTokensWork = () => {
    return (_jsxs("div", { className: "space-y-8", children: [_jsxs("div", { className: "text-center bg-gradient-to-r from-yellow-50 to-amber-50 rounded-xl p-8", children: [_jsx(Coins, { className: "w-16 h-16 text-yellow-600 mx-auto mb-4" }), _jsx("h1", { className: "text-3xl font-bold text-yellow-900 mb-4", children: "Faith Tokens: Rewards for Faithful Creators" }), _jsx("p", { className: "text-lg text-yellow-700 max-w-2xl mx-auto mb-6", children: "Our token system rewards engagement, loyalty, and community participation. Earn tokens through daily actions and redeem them for real value." }), _jsx(Link, { to: "/tokens", children: _jsx(Button, { className: "bg-yellow-600 hover:bg-yellow-700", children: "Start Earning Tokens" }) })] }), _jsxs("div", { children: [_jsx("h2", { className: "text-2xl font-bold text-anchor-900 mb-6 text-center", children: "How to Earn Faith Tokens" }), _jsx("div", { className: "grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6", children: [
                            {
                                title: 'Daily Check-in',
                            ...
                            return (_jsx(Card, { className: `bg-${method.color}-50 border-${method.color}-200`, children: _jsxs(CardContent, { className: "text-center p-6", children: [_jsx(IconComponent, { className: `w-8 h-8 text-${method.color}-600 mx-auto mb-3` }), _jsx("h3", { className: `font-semibold text-${method.color}-800 mb-2`, children: method.title }), _jsx("p", { className: `text-sm text-${method.color}-600 mb-2`, children: method.description }), _jsxs("div", { className: `bg-${method.color}-100 text-${method.color}-700 px-2 py-1 rounded text-xs font-bold`, children: ["+", method.tokens] })] }) }, method.title));
                            ...
                            return (_jsx(Card, { className: `bg-${tier.color}-50 border-${tier.color}-200`, children: _jsxs(CardContent, { className: "text-center p-6", children: [_jsx(IconComponent, { className: `w-8 h-8 text-${tier.color}-600 mx-auto mb-3` }), _jsxs("h3", { className: `font-semibold text-${tier.color}-800 mb-2`, children: [tier.tier, " Member"] }), _jsxs("div", { className: `bg-${tier.color}-100 text-${tier.color}-700 px-2 py-1 rounded text-sm font-bold mb-3`, children: [tier.monthlyCap, " tokens/month"] }), _jsx("ul", { className: `text-xs text-${tier.color}-600 space-y-1`, children: tier.features.map((feature, index) => (_jsxs("li", { children: ["\u2022 ", feature] }, index))) })] }) }, tier.tier));
                        }) })] }), _jsxs("div", { children: [_jsx("h2", { className: "text-2xl font-bold text-anchor-900 mb-6 text-center", children: "Redeem for Real Value" }), _jsx("div", { className: "grid grid-cols-1 md:grid-cols-3 gap-6", children: [
                            {
                        ...
                        ].map((redemption) => (_jsx(Card, { children: _jsxs(CardContent, { className: "text-center p-6", children: [_jsx("div", { className: "text-4xl mb-3", children: redemption.icon }), _jsx("h3", { className: "font-semibold text-anchor-800 mb-2", children: redemption.title }), _jsx("p", { className: "text-sm text-anchor-600 mb-3", children: redemption.description }), _jsx("div", { className: "bg-primary-100 text-primary-700 px-3 py-1 rounded font-bold text-sm", children: redemption.cost })] }) }, redemption.title))) })] }), _jsx(Card, { className: "bg-blue-50 border-blue-200", children: _jsxs(CardContent, { className: "p-8 text-center", children: [_jsx(Shield, { className: "w-12 h-12 text-blue-600 mx-auto mb-4" }), _jsx("h3", { className: "text-xl font-semibold text-blue-900 mb-4", children: "Fair Play Guarantee" }), _jsx("p", { className: "text-blue-700 max-w-2xl mx-auto mb-4", children: "Our token system is designed to reward genuine engagement and prevent abuse. We have daily, weekly, and monthly caps to ensure fairness for all members." }), _jsxs("div", { className: "grid grid-cols-1 md:grid-cols-3 gap-4 text-sm", children: [_jsxs("div", { className: "bg-white rounded-lg p-3", children: [_jsx("div", { className: "font-semibold text-blue-800", children: "Daily Limits" }), _jsx("div", { className: "text-blue-600", children: "Prevent token farming" })] }), _jsxs("div", { className: "bg-white rounded-lg p-3", children: [_jsx("div", { className: "font-semibold text-blue-800", children: "Email Verified" }), _jsx("div", { className: "text-blue-600", children: "One account per person" })] }), _jsxs("div", { className: "bg-white rounded-lg p-3", children: [_jsx("div", { className: "font-semibold text-blue-800", children: "Tier Based" }), _jsx("div", { className: "text-blue-600", children: "Higher tiers = better rewards" })] })] })] }) }), _jsx("div", { className: "text-center", children: _jsx(Link, { to: "/tokens", children: _jsx(Button, { size: "lg", className: "bg-primary-600 hover:bg-primary-700", children: "Start Earning Faith Tokens Today" }) }) })] }));
};
